...
| Q10 | Process | Run original vs rewrite suite; compare **SPOF labels**, not the score | P0 | **blocked** | dogfood | `eval/stability.ts`, `eval/hinge-labels.ts`, `eval/golden-variants/*`, `npm run eval:stability` | **Instrument complete 2026-07-30, only the run is blocked — on owner provider credentials.** 5 base fixtures × 3 rewrites (`para` full paraphrase, `strip` proper nouns/comparators/places removed, `flip` same facts reversed + reframed) = 20 analyses, 60+ provider calls. Verdict is automatic (`same` / `partial` / `shift` / `unmatched`) by comparing SPOF **themes**, so no manual step stands between a run and a number; the rollup breaks shifts down per rewrite kind, which is what names the cause. `BIF_REF=<baseline run_id>` diffs against 2026-07-16 labels; `BIF_STABILITY_GATE=1` exits non-zero on any shift. Score cannot substitute even at 48/52 (see Q13) — it measures one report, not stability across reports |
...
**Counts:** todo **1** (E19) · doing **1** (Q11 freeze) · blocked **1** (Q10 — instrument complete, needs owner credentials only) · **done (suite + implement batch + Q8/Q9 provenance & trace + Q12 trace reader + Q13 rubric 48/52 + Q14 stability instrument + S5 type-lab deleted)** · see master board
